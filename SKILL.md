...
这个 skill 只负责“最新趋势研究 + Markdown 成稿”。脚本会尽量给每篇文章补一张代表图：优先使用 Kimi 返回的 `representative_image_url`，如果为空，再从证据来源页抓 `og:image`、`twitter:image` 或正文首图。抓不到时就省略图片行。如果用户还要网页、长图或 PDF，再基于这些 `.md` 继续扩展。
